...
// ===== COMPACT ATTENDANCE MOBILE V2 START =====
(() => {
    const STATUS_RE = /^(HADIR|TERLAMBAT|IZIN|SAKIT|ALFA|BELUM ABSEN)$/i;
    const TIME_RE = /^\d{1,2}[.:]\d{2}$/;
    const CLASS_RE = /^(X|XI|XII)[.\s-]?\d+$/i;
    const DATE_RE = /\b\d{1,2}\s+(Jan|Feb|Mar|Apr|Mei|Jun|Jul|Agu|Sep|Okt|Nov|Des)[a-z]*\s+\d{4}\b/i;
    ...
    function classifyRow(row) {
        ...
        const text = cell => clean(cell.textContent);
        const actionCell = cells.find(cell =>
            cell.querySelector("button, a, [role='button']")
        );

        const statusCell = cells.find(cell => STATUS_RE.test(text(cell)));
        const timeCell = cells.find(cell => TIME_RE.test(text(cell)));
        const classCell = cells.find(cell => CLASS_RE.test(text(cell)));
        const dateCell = cells.find(cell => DATE_RE.test(text(cell)));
        ...
        const nameCell = cells
            ...
            .filter(cell => !/^\d+$/.test(text(cell)))
            .sort((a, b) => text(b).length - text(a).length)[0];

        if (!nameCell || !statusCell) return null;

        row.classList.add("attendance-compact-row");
        nameCell.classList.add("ac-name");
        statusCell.classList.add("ac-status");

        if (actionCell) actionCell.classList.add("ac-action");
        if (classCell) classCell.classList.add("ac-class");
        if (timeCell) timeCell.classList.add("ac-time");
        if (dateCell) dateCell.classList.add("ac-date");

        const status = text(statusCell).toUpperCase();
        statusCell.dataset.status = status;

        nameCell.dataset.acClass = classCell ? text(classCell).toUpperCase() : "";
        nameCell.dataset.acTime = timeCell ? text(timeCell).replace(".", ":") : "";
        nameCell.dataset.acDate = dateCell ? text(dateCell) : "";

        let meta = nameCell.querySelector(".ac-mobile-meta");
        if (!meta) {
            meta = document.createElement("span");
            meta.className = "ac-mobile-meta";
            nameCell.appendChild(meta);
        }

        row.classList.add("ac-ready");
        return { row, nameCell, date: nameCell.dataset.acDate };
    }
    ...
    function processTable(table) {
        ...
        const records = rows.map(classifyRow).filter(Boolean);
        if (!records.length) return;

        table.classList.add("attendance-compact-table");

        const dates = [...new Set(
            records.map(record => record.date).filter(Boolean)
        )];

        const oneDate = dates.length === 1 ? dates[0] : "";

        let dateGroup = table.previousElementSibling;
        if (!dateGroup?.classList.contains("attendance-date-group")) {
            dateGroup = document.createElement("div");
            dateGroup.className = "attendance-date-group";
            table.before(dateGroup);
        }

        if (oneDate) {
            dateGroup.textContent = oneDate;
            dateGroup.hidden = false;
        } else {
            dateGroup.hidden = true;
        }

        records.forEach(({ nameCell }) => {
            const parts = [
                nameCell.dataset.acClass,
                nameCell.dataset.acTime
            ].filter(Boolean);

            if (!oneDate && nameCell.dataset.acDate) {
                parts.push(nameCell.dataset.acDate);
            }

            const meta = nameCell.querySelector(".ac-mobile-meta");
            if (meta) meta.textContent = parts.join(" • ");
        });
    }

    function refreshCompactTables() {
...
// ===== COMPACT ATTENDANCE MOBILE V2 END =====
